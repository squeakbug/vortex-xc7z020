...
`define CACHE_E_TAG_WIDTH(mshr_size, num_banks, mem_ports, uuid_width) \
...
`define CACHE_NC_E_TAG_WIDTH(mshr_size, num_banks, num_reqs, mem_ports, line_size, word_size, tag_width, uuid_width) \
        (`AX(`CACHE_E_TAG_WIDTH(mshr_size, num_banks, mem_ports, uuid_width), `CACHE_BYPASS_TAG_WIDTH(num_reqs, mem_ports, line_size, word_size, tag_width)) + 1)
...
`define CACHE_CLUSTER_E_ARB_TAG(tag_width, num_caches) \
        (tag_width + `ARB_SEL_BITS(`UP(num_caches), 1))

`define CACHE_CLUSTER_E_TAG_WIDTH(mshr_size, num_banks, mem_ports, num_caches, uuid_width) \
        `CACHE_CLUSTER_E_ARB_TAG(`CACHE_E_TAG_WIDTH(mshr_size, num_banks, mem_ports, uuid_width), num_caches)

`define CACHE_CLUSTER_BYPASS_E_TAG_WIDTH(num_reqs, mem_ports, line_size, word_size, tag_width, num_inputs, num_caches) \
        `CACHE_CLUSTER_E_ARB_TAG(`CACHE_BYPASS_TAG_WIDTH(num_reqs, mem_ports, line_size, word_size, `CACHE_CLUSTER_CORE_ARB_TAG(tag_width, num_inputs, num_caches)), num_caches)

`define CACHE_CLUSTER_NC_E_TAG_WIDTH(mshr_size, num_banks, num_reqs, mem_ports, line_size, word_size, tag_width, num_inputs, num_caches, uuid_width) \
        `CACHE_CLUSTER_E_ARB_TAG(`CACHE_NC_E_TAG_WIDTH(mshr_size, num_banks, num_reqs, mem_ports, line_size, word_size, `CACHE_CLUSTER_CORE_ARB_TAG(tag_width, num_inputs, num_caches), uuid_width), num_caches)

`define TO_FULL_ADDR(x) {x, (`E_ADDR_WIDTH-$bits(x))'(0)}
...
`define ASSIGN_VX_E_BUS_IF(dst, src) \
...
`define ASSIGN_VX_E_BUS_RO_IF(dst, src) \
    assign dst.req_valid = src.req_valid; \
...
`define ASSIGN_VX_E_BUS_IF_EX(dst, src, TD, TS, UUID) \
    /* verilator lint_off GENUNNAED */ \
    assign dst.req_valid = src.req_valid; \
